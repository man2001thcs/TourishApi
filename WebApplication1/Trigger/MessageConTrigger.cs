...
namespace WebApplication1.Trigger
{
    public class MessageConTrigger : IAfterSaveTrigger<UserMessageCon>
    {
        ...
        public Task AfterSave(ITriggerContext<UserMessageCon> context, CancellationToken cancellationToken)
        {
            ...
            {
                var connectionCount = this._dbContext.MessageConList.Where(messageCon => messageCon.UserId == context.Entity.UserId).Count();
                if (connectionCount > 5)
                {
                    var conList = this._dbContext.MessageConList
                ...
                this._dbContext.MessageConList.Where(u => u.UserId == context.Entity.UserId && u.Connected)
